	...
	public String auctionAnalysis(Model model, @RequestParam(value = "bidDate", defaultValue = "12.01") String bidDate)
		...
		for (AuctionList tmpData : _auctionList) {
			...
			if (tmpData.getAppraisedvalue().length() > 0) {
				auctionListVO.setAppraisedvalue(String.valueOf((int)Double.parseDouble(tmpData.getAppraisedvalue()) / 10000));
			}

			if (tmpData.getLowestvalue().length() > 0) {
				auctionListVO.setLowestvalue(String.valueOf((int)Double.parseDouble(tmpData.getLowestvalue()) / 10000));
			}

			if (tmpData.getSalevalue().length() > 0) {
				auctionListVO.setSalevalue(String.valueOf((int)Double.parseDouble(tmpData.getSalevalue()) / 10000));
			}

...
public interface SidogusRepository extends JpaRepository<Sidogus, Integer>{
	...
	@Query(value="select concat(e1.ind,'\\\\',e2.ind,'\\\\',e3.ind,'\\\\') filepath from Sidos e1, Sidogus e2, Newdongs e3 where e1.ind = :ind and e1.ind = e2.si_ind and e2.si_ind = e3.si_ind and e2.ind = e3.sigogus_ind order by e1.ind, e2.ind, e3.ind asc", nativeQuery=true)
	List<String> findDongFilePath(@Param("ind") int ind);
	
	@Query(value="select concat(si_ind, '\\\\', ind, '\\\\') filePath from sidogus where si_ind = :si_ind order by ind asc", nativeQuery=true)
